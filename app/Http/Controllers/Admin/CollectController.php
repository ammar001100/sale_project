...
use App\Http\Requests\CollectTransactionRequest;
...
class CollectController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $mov_type = Mov_type::Selection()->where(['active' => 1, 'in_screen' => 2, 'is_private_intemal' => 0])->get();
            return view('admin.treasury_transactions.index', compact('data', 'admin_shift', 'account', 'money', 'mov_type'));
        } catch (\Exception $ex) {
     ...
     */
    public function store(CollectTransactionRequest $request)
    {
        try {
            ...
            $data_insert['isal_number'] = $treasury->last_isal_collect + 1;
            ...
            $data_insert['money'] = $request->money; //مدين
            $data_insert['mov_date'] = $request->mov_date;
            $data_insert['is_approved'] = 1;
            $data_insert['is_account'] = 1;
            $data_insert['money_for_account'] = $request->money * (-1); //دائن
            ...
            if ($flag) {
                Treasury::selection()->where('id', $request->treasury_id)
                    ->update(['last_isal_collect' => $data_insert['isal_number']]);
            }
            ...
            } else {}session()->flash('success', 'تم اضافة بيانات التحصيل بنجاح');

            return redirect()->route('collect_transactions.index');
        } catch (\Exception $ex) {
            session()->flash('error', 'حدث خطاء ما' . ' => ' . $ex->getMessage());
